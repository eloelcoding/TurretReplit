...
config = {
  // Dev mode from environment variable (injected by server)
  devMode: typeof window !== 'undefined' && window.__DEV_MODE__ === true,
  ...
  money: 1000,
  enemyHealth: 5,
  safeRegenSpeed: 100,  // Safe regeneration interval in ms (lower = faster)
  safeCooldownTime: 2000,  // Safe cooldown after collection in ms
  
  // Centralized prices configuration
  prices: {
    // Base turret prices
    turret: 40,
    twinGun: 60,
    ...
    // Shop item prices
    healthItem: 250,
    maxHealthUpgrade: 500,
    
    // Upgrade prices [level 1->2, level 2->3]
    upgrades: {
      turret: [40, 80],
      twinGun: [60, 120],
      cannon: [80, 160],
  ...
  // Initialize sound functions as no-ops for dev (disabled to avoid loading issues)
  _soundFiles: {},
  _currentMusic: null,
  playSound: function(soundName) {
    // Disabled in dev - do nothing
    return;
  },
  ...
  setMusicVolume: function(volume) {
    // Still update the volume setting, but don't play music
  ...
  preload() {
    ...
    // Sounds disabled in dev - skip loading to avoid issues
    // Just set empty sound files object
    config._soundFiles = {};
    ...
    // Sound functions disabled in dev to avoid loading issues
    config.playSound = function(soundName) {
      // Disabled in dev - do nothing
      return;
    }

    // Music control - use config._currentMusic so it's accessible everywhere
    config._currentMusic = null;
