class TurretBase {
  ...
  twirl() {
    ...
    createjs.Tween.get(this)
      .to({ level: this.level + 1, angleToMob: originalAngle + PI * 4 }, 2500, createjs.Ease.getPowInOut(3))
      // .to({ angleToMob: this.angleToMob + PI * 4 }, 100, createjs.Ease.getPowInOut(1))
      .call(() => {
        this.angleToMob = originalAngle;
        createjs.Tween.get(this)
          .to({ angleToMob: this.getAngleToMob() }, 100, createjs.Ease.getPowInOut(1))
          .call(() => {
  ...
  getAngleToMob() {
    ...
    if (enemyToShoot) {
      ...
      // return atan2(dy, dx);
      var smoothing = 0.3;
      return smoothing * this.angleToMob + atan2(dy, dx) * (1 - smoothing);
    }
