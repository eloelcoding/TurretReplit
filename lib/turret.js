class Unit {
  ...
  drawFuturisticTurret() {
    ...
    if (level >= 3) {
      ...
      // Level 2 has double barrel
      this.drawBarrel(size * 0.25, -barrelWidth * 0.4, barrelLength, barrelWidth * 0.6, colors);
      this.drawBarrel(size * 0.25, barrelWidth * 0.4, barrelLength, barrelWidth * 0.6, colors);
    } else {
      // Level 1 has single barrel
...
class TurretBase extends Unit {
  ...
  shoots() {
    ...
    // Create projectiles based on level (1 at level 1, 2 at level 2, 3 at level 3)
    let projectileColor = this.getProjectileColor();
    let level = Math.floor(this.level);
    let numProjectiles = level; // 1, 2, or 3 projectiles
    
    // Calculate barrel positions matching the visual barrels
    // Barrel positions are relative to turret center, rotated by angleToMob
    let barrelPositions = [];
    let size = 280 * 0.2 * (config.scale || 1); // Scaled barrel size
    let barrelWidth = size * 0.18;
    let barrelStartX = size * 0.25;
    
    if (numProjectiles === 1) {
      // Single barrel at center
      barrelPositions.push({ x: barrelStartX, y: 0 });
    } else if (numProjectiles === 2) {
      // Two barrels side by side
      barrelPositions.push({ x: barrelStartX, y: -barrelWidth * 0.4 });
      barrelPositions.push({ x: barrelStartX, y: barrelWidth * 0.4 });
    } else if (numProjectiles === 3) {
      // Three barrels (top, center, bottom)
      barrelPositions.push({ x: barrelStartX, y: -barrelWidth * 0.7 });
      barrelPositions.push({ x: barrelStartX, y: 0 });
      barrelPositions.push({ x: barrelStartX, y: barrelWidth * 0.7 });
    }
    
    // Fire all projectiles simultaneously from their barrel positions
    // Rotate barrel positions to match turret rotation
    let cosAngle = cos(this.angleToMob);
    let sinAngle = sin(this.angleToMob);
    
    for (let i = 0; i < numProjectiles; i++) {
      let barrelPos = barrelPositions[i];
      // Rotate barrel position to match turret angle
      let rotatedX = barrelPos.x * cosAngle - barrelPos.y * sinAngle;
      let rotatedY = barrelPos.x * sinAngle + barrelPos.y * cosAngle;
      
      // Fire projectile from barrel position
      effectsManager.addProjectile(
        this.x + rotatedX, 
        this.y + rotatedY, 
        target, 
        this.damage(), 
...
class Turret extends TurretBase {
  constructor(x, y, z) {
    super(x, y)
    this.z = z
    this.price = config.prices.turret;
  ...
  config() {
    return {
      ...
      imageMap: "turretMap",
      upgradePrice: config.prices.upgrades.turret
    }
  }
...
class TwinGun extends TurretBase {
  constructor(x, y, z) {
    super(x, y)
    this.z = z
    this.price = config.prices.twinGun;
  ...
  config() {
    return {
      ...
      imageMap: "twinGunMap",
      upgradePrice: config.prices.upgrades.twinGun
    }
  }
...
class Cannon extends TurretBase {
  constructor(x, y, z) {
    super(x, y)
    this.z = z
    this.price = config.prices.cannon;
  ...
  config() {
    return {
      ...
      imageMap: "cannonMap",
      upgradePrice: config.prices.upgrades.cannon
    }
  }
...
class Safe extends Unit {
  constructor(x, y, z) {
    super(x, y)
    this.z = z
    this.price = config.prices.safe;
    this.amount = 0;
    
    // State: 'accumulating', 'full', 'cooldown'
    this.state = 'accumulating';
    this.cooldownTime = config.safeCooldownTime; // Configurable cooldown time
  ...
  range() {
    // Safe doesn't shoot, so return very small range (essentially invisible)
    return 1;
  }
  ...
  deposit() {
    ...
    if (this.amount >= limit) {
      this.amount = limit;
      this.state = 'full'; // Ready to collect, but no time limit
      // Stop accumulating when full
      if (this._interval) {
        clearInterval(this._interval);
        this._interval = null;
      }
  ...
  startCooldown() {
    ...
    this.amount = 0;
    // Stop accumulating during cooldown
    if (this._interval) {
      clearInterval(this._interval);
      this._interval = null;
    }
  ...
  updatePayRate() {
    // Use config setting for regeneration speed
    var payInterval = config.safeRegenSpeed || this.getCurrentSetting("interval");
    if (this._interval) {
      clearInterval(this._interval);
    }
  ...
  collect() {
    if (this.state === 'full' && this.amount > 0) {
  ...
  doubleClicked() {
    // Use a larger click radius for Safe (it's bigger than regular turrets)
    var clickRadius = 50; // Larger than the default 30
    if (dist(mouseX, mouseY, this.x, this.y) < clickRadius) {
      return this.collect();
    }
    return false;
  }
  ...
  mouseClicked() {
    // Single click no longer collects - only double click
    return false;
  }
  ...
  upgrade() {
    ...
    if (game.money > upgradePrice) {
      ...
      createjs.Tween.get(this)
        ...
        .call(() => {
          this.active = true;
          // Reset to accumulating state after upgrade
          this.startAccumulating();
        });
    }
  }
  ...
  config() {
    return {
      ...
      limit: [100, 200, 300],
      imageMap: "safeMap",
      upgradePrice: config.prices.upgrades.safe,
      range: [125, 125, 100],
    }
  }
  ...
  drawMoneyGenerator() {
    ...
    // State-based visual feedback - only show if Safe is placed on map (not in shop preview)
    // Check if this Safe is actually in the game's turrets array
    let isPlaced = game && game.turrets && game.turrets.includes(this);
    
    if (isPlaced) {
      if (this.state === 'full') {
        // Subtle glow when full and ready to collect
        let pulseAlpha = 50 + sin(frameCount * 0.15) * 30;
        fill(0, 255, 200, pulseAlpha);
        noStroke();
        ellipse(0, 0, size * 1.1, size * 1.1);
        
        // Subtle border to indicate ready
        ...
        textSize(14);
        textAlign(CENTER);
        text("2x Click", 0, -size * 0.35);
      } else if (this.state === 'cooldown') {
        // Greyed out appearance during cooldown
        ...
        noStroke();
        rect(0, size * 0.3, size * 0.6 * cooldownProgress, 6, 3);
        
        // Cooldown text
        fill(150, 150, 150);
        textSize(12);
        textAlign(CENTER);
        let timeLeft = Math.ceil((this.cooldownTime - (millis() - this.cooldownStart)) / 1000);
        text(timeLeft + "s", 0, size * 0.4);
        
        drawingContext.globalAlpha = 1;
      }
      
      // Holographic credit amount display (when accumulating or full)
      if (this.amount > 0 && (this.state === 'accumulating' || this.state === 'full')) {
        // Hologram effect
  ...
  draw() {
    ...
    // Update state machine only if placed on map (not shop preview)
    let isPlaced = game && game.turrets && game.turrets.includes(this);
    if (isPlaced) {
      this.update();
    }
    
    // Use Unit's draw method but with our custom display
    push();
